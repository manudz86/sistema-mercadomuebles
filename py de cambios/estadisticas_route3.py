...
def exportar_reposicion():
    ...
    # Stock disponible = stock_actual - pendiente de entrega
    stock_disp_rows = query_db("""
        SELECT
            pb.sku,
            pb.stock_actual - COALESCE(SUM(
                CASE WHEN v.estado_entrega NOT IN ('entregada', 'cancelada')
                THEN iv_p.cantidad ELSE 0 END
            ), 0) as stock_disponible
        FROM productos_base pb
        LEFT JOIN items_venta iv_p ON iv_p.sku = pb.sku
        LEFT JOIN ventas v ON v.id = iv_p.venta_id
        GROUP BY pb.sku, pb.stock_actual
    """)
    stock_disponible_map = {r['sku']: int(r['stock_disponible'] or 0) for r in stock_disp_rows}
