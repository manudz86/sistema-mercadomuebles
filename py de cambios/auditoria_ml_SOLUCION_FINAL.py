...
def auditoria_ml():
    ...
    try:
        ...
        for pub in publicaciones_db:
            ...
            # Si el SKU termina en Z y no existe, buscar sin la Z
            if not stock_info and sku.endswith('Z'):
                sku_sin_z = sku[:-1]  # Quitar la Z del final
                stock_info = stock_por_sku.get(sku_sin_z)
                if stock_info:
                    print(f"DEBUG: {sku} → usando stock de {sku_sin_z}")
