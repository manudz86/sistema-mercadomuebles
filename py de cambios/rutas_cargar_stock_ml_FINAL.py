...
# VERSIÓN FINAL - USA cargar_ml_token() como las alertas
# ============================================================================
...
def buscar_sku_ml():
    ...
    # Usar mla_id en vez de mla (según tu esquema)
    query = """
        SELECT mla_id, titulo, stock_actual, estado
        FROM sku_mla_mapeo
        WHERE sku = %s
        ORDER BY mla_id
    """
...
def quitar_demora_mla():
    ...
    publicaciones = query_db(
        "SELECT mla_id, titulo, stock_actual, estado FROM sku_mla_mapeo WHERE sku = %s",
        (sku,)
    )
...
def quitar_demora_masivo():
    ...
    mlas = query_db("SELECT mla_id FROM sku_mla_mapeo WHERE sku = %s", (sku,))
...
def cargar_stock_mla():
    ...
    try:
        ...
        # Usar la función helper actualizar_stock_ml si existe
        # Si no, hacer request directo
