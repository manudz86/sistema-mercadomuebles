...
@app.route('/cargar-stock/escanear-remito', methods=['POST'])
@login_required
def escanear_remito():
    """Escanea imágenes de remito con IA (Claude vision) y extrae productos/cantidades."""
    import base64
    ...
    if not ANTHROPIC_API_KEY:
        return jsonify({'success': False, 'error': 'ANTHROPIC_API_KEY no configurada en el servidor'}), 500

    try:
        # Armar contenido multimodal: primero todas las imágenes, luego el prompt
        content = []
        for archivo in archivos:
            imagen_bytes = archivo.read()
            data_b64 = base64.b64encode(imagen_bytes).decode('utf-8')
        ...
        if not api_resp.ok:
            return jsonify({'success': False, 'error': f'Error API Anthropic: {api_resp.status_code} - {api_resp.text[:200]}'}), 500

        texto = api_resp.json()['content'][0]['text'].strip()

        # Limpiar posibles markdown fences en la respuesta
        if '```' in texto:
        ...
        items_remito = json.loads(texto)

        # Consolidar por codigo_material (por si hay duplicados entre hojas del mismo remito)
        consolidado = {}
        for item in items_remito:
            codigo = (item.get('codigo_material') or '').strip()
            cantidad = int(item.get('cantidad') or 0)
            descripcion = (item.get('descripcion') or '').strip()
            if not codigo or cantidad <= 0:
                continue
            if codigo in consolidado:
                consolidado[codigo]['cantidad'] += cantidad
            else:
                consolidado[codigo] = {'codigo_material': codigo, 'descripcion': descripcion, 'cantidad': cantidad}

        # Resolver SKUs desde cannon_productos
        resultados = []
        for codigo, item in consolidado.items():
            cp = query_one(
                'SELECT codigo_material, descripcion, sku FROM cannon_productos WHERE codigo_material = %s',
                (codigo,)
            )
            if cp and cp.get('sku'):
                sku = cp['sku']
                pb = query_one(
                    'SELECT nombre, stock_actual, COALESCE(stock_full, 0) as stock_full FROM productos_base WHERE sku = %s',
                    (sku,)
                )
                resultados.append({
                    'codigo_material': codigo,
                    'descripcion_remito': item['descripcion'],
                    'cantidad': item['cantidad'],
                    'sku': sku,
                    'nombre': pb['nombre'] if pb else sku,
                    'stock_actual': int(pb['stock_actual']) if pb else 0,
                    'resuelto': True
                })
            else:
                resultados.append({
                    ...
                    'stock_actual': None,
                    'resuelto': False,
                    'en_cannon_productos': cp is not None
                })

        return jsonify({'success': True, 'items': resultados})

    except json.JSONDecodeError as e:
        texto_preview = texto[:300] if 'texto' in dir() else '(no disponible)'
        return jsonify({'success': False, 'error': f'Error parseando respuesta IA: {str(e)}. Respuesta: {texto_preview}'}), 500
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)}), 500


@app.route('/cargar-stock/registrar-mapeo-remito', methods=['POST'])
@login_required
def registrar_mapeo_remito():
    """Guarda el mapeo codigo_material → SKU en cannon_productos para futuros escaneos."""
    data = request.get_json() or {}
    codigo_material = (data.get('codigo_material') or '').strip()
    sku = (data.get('sku') or '').strip().upper()
    descripcion = (data.get('descripcion') or '').strip()

    if not codigo_material or not sku:
        return jsonify({'success': False, 'error': 'Datos incompletos'}), 400

    pb = query_one('SELECT nombre FROM productos_base WHERE sku = %s', (sku,))
    if not pb:
        return jsonify({'success': False, 'error': f'SKU {sku} no encontrado en sistema'}), 400

    execute_db('''
        INSERT INTO cannon_productos (codigo_material, descripcion, sku)
        VALUES (%s, %s, %s)
        ON DUPLICATE KEY UPDATE sku = VALUES(sku)
    ''', (codigo_material, descripcion, sku))

    return jsonify({'success': True, 'nombre': pb['nombre']})
...
def _participar_lightning_una(access_token, mla, promotion_id):
    ...
    stock. Se usa el MENOR descuento exigido (max_discounted_price) y el MÁXIMO stock
    permitido (tope del rango). Devuelve dict {ok, tipo, deal_price, original_price, pct, stock, error}."""
    def _safe_json(resp):
        try:
            return resp.json()
        except Exception:
    ...
    # candidato de esta publi para esta relámpago
    cand = None
    try:
        rg = ml_request('get', f'https://api.mercadolibre.com/seller-promotions/items/{mla}',
                        access_token, params={'app_version': 'v2'})
        for p in (_safe_json(rg) or []):
            if isinstance(p, dict) and p.get('type') == 'LIGHTNING' and \
               p.get('id') == promotion_id and p.get('status') == 'candidate' and p.get('ref_id'):
                cand = p
                break
    except Exception:
        pass
    if not cand:
        return {'ok': False, 'error': 'Sin candidato LIGHTNING (ya activa o no aplica)'}

    # precio: menor descuento exigido (máximo precio permitido)
    maxp = cand.get('max_discounted_price')
    if not maxp:
        return {'ok': False, 'error': 'La relámpago no informó rango de precio'}
    deal_price = int(round(float(maxp)))

    # stock: máximo permitido dentro del rango (ML rechaza el tope exacto → tope-1)
    ...
    if ra.status_code in (200, 201):
        orig = r.get('original_price') if isinstance(r, dict) else None
        precio_final = (r.get('price') if isinstance(r, dict) else None) or deal_price
        pct = None
        try:
            if orig and precio_final:
                pct = f"{(float(orig) - float(precio_final)) / float(orig) * 100:.1f}".replace('.', ',')
            ...
            pass
        return {'ok': True, 'tipo': 'LIGHTNING', 'deal_price': precio_final,
                'original_price': orig, 'pct': pct, 'stock': stock_commit}
    msg = (r.get('message') if isinstance(r, dict) else None) or f'HTTP {ra.status_code}'
    if isinstance(r, dict) and r.get('cause'):
        detalle = '; '.join(c.get('error_message', '') for c in r['cause'] if c.get('error_message'))
        if detalle:
...
def iniciar_scheduler():
    try:
        ...
        # si no hay datos nuevos, reintenta cada 1 hora (hasta 8 veces).
        try:
            import competencia_v2_bp as _cv2
            _cv2._SCHEDULER = scheduler        # para que el job pueda reprogramar reintentos
            scheduler.add_job(_cv2.job_actualizar_rt, 'cron', hour=16, minute=30,
                             id='job_rt_update_diario', replace_existing=True)
            print("[COMPETENCIA-V2] Job Real Trends agendado: 16:30 UTC (13:30 AR), reintento c/1h si no hay datos")
        except Exception as e:
...
def costos_descuentos():
    ...
    if request.method == 'POST':
        data = request.get_json() or {}
        # Override de precio CTR80
        if 'ctr80_precio' in data:
            try:
                valor = data.get('ctr80_precio') or ''
                execute_db("""
                    INSERT INTO configuracion (clave, valor) VALUES ('ctr80_precio_cannon', %s)
                    ON DUPLICATE KEY UPDATE valor = VALUES(valor)
                """, (str(valor).strip(),))
                return jsonify(ok=True)
            except Exception as e:
